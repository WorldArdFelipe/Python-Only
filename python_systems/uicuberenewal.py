...
def FReturnInfo(func,index):
	...
	if app.ENABLE_CUBE_RENEWAL_GEM_WORLDARD and not app.ENABLE_CUBE_RENEWAL_COPY_WORLDARD:
		(vnum_reward,count_reward,item_reward_stackable,vnum_material_1,count_material_1,vnum_material_2,count_material_2,vnum_material_3,count_material_3,vnum_material_4,count_material_4,vnum_material_5,count_material_5,gold,percent,category,gem) = cube_renewal.GetDates(index)
	
	elif not app.ENABLE_CUBE_RENEWAL_GEM_WORLDARD and app.ENABLE_CUBE_RENEWAL_COPY_WORLDARD:
	...
	elif app.ENABLE_CUBE_RENEWAL_GEM_WORLDARD and app.ENABLE_CUBE_RENEWAL_COPY_WORLDARD:
		(vnum_reward,count_reward,item_reward_stackable,vnum_material_1,count_material_1,vnum_material_2,count_material_2,vnum_material_3,count_material_3,vnum_material_4,count_material_4,vnum_material_5,count_material_5,gold,percent,category,gem,index_copy) = cube_renewal.GetDates(index)
	...
	if app.ENABLE_CUBE_RENEWAL_GEM_WORLDARD:
		info["gem"] = gem
...
class CubeRenewalWindows(ui.ScriptWindow):
	...
	def UpdateMaterialCount(self):
		...
		if app.ENABLE_CUBE_RENEWAL_GEM_WORLDARD:
			self.gem_text.SetText(localeInfo.NumberToMoneyWA(self.selectedItem.GetGem()*count))
			self.gem_text.Show()

			if int(self.selectedItem.GetGem()*count) < player.GetGaya():
	...
	def OverInItem(self, index):
		...
		self.toolTip.AddItemData(vnum,metinSlot,AttrList)
