...
class MailBox(ui.ScriptWindow):
	...
	def LoadWindow(self):
		try:
			...
			PythonScriptLoader.LoadScriptFile(self, "Modulo/mailbox.py")
		except:
			import exception
	...
	def __DeleteMails(self):
		if self.CheckMails() == False:
			self.popup.SetText("No hay Mensajes")
			self.popup.Open()
			return

		if self.CheckAceptMails() == True:
			questionDialog = uiCommon.QuestionDialog2()
			questionDialog.SetText1("Borrar todos los mensajes")
			questionDialog.SetText2("Los mensajes con adjunto no se borraran.")
	...
	def __RecibeMails(self):
		if self.CheckAceptMails() == False:
			self.popup.SetText("No hay mensajes con adjunto en el buzon")
			self.popup.Open()
			return
...
class MailBoxRead(ui.ScriptWindow):
	...
	def LoadWindow(self):
		try:
			PythonScriptLoader = ui.PythonScriptLoader()
			PythonScriptLoader.LoadScriptFile(self, "Modulo/postread.py")
		except:
			import exception
	...
	def __DeleteMail(self):
		if self.CheckMail() == False:
			chat.AppendChat(1,"No se ha podido borrar el mensaje. Acepta primero el adjunto.")
			return
	...
	def __RecibeMail(self):
		if self.CheckMail() == False:
			if self.GOLD != 0:
				...
				questionDialog.SetText("Aceptar adjunto (Previa deduccion de un 5%  de tasas por Yang)")
	...
	def OverInItem(self):
		...
		if app.ENABLE_TRANSMUTATION_MAIL_BOX_SYSTEM:
			...
			self.toolTip.AddItemDataMailBox(self.ITEM,self.list_socket[0],self.list_attr[0],nameTransmutation)
		else:
			self.toolTip.AddItemDataMailBox(self.ITEM,self.list_socket[0],self.list_attr[0])
...
class MailBoxSend(ui.ScriptWindow):
	...
	def LoadWindow(self):
		try:
			PythonScriptLoader = ui.PythonScriptLoader()
			PythonScriptLoader.LoadScriptFile(self, "Modulo/postwrite.py")
		except:
			import exception
		...
		dlgPickMoney.SetTitleName("Exchange")
	...
	def __OnSelectEmptySlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			if item.IsAntiFlag(item.ANTIFLAG_GIVE):
				chat.AppendChat(chat.CHAT_TYPE_INFO, "[MailBox]No puede enviar ese item.")
				return
	...
	def __SendMail(self):
		...
		if self.COMPROBACION == False: #Check Nombre
			self.ChatW("Verifica el nombre del destinario haciendo click en <Comprobar Destinario> ")
			return
		if len(asunto) == 0:
			self.ChatW("Verifica el espacio <Asunto> ")
			return
		elif len(descrip) == 0:
			self.ChatW("Verifica el espacio <Descripcion> ")
			return

		if asunto.find("'") != -1 or asunto.find('"') != -1:
			self.ChatW("Verifique los caracteres de <Asunto>")
			return

		if descrip.find("'") != -1 or descrip.find('"') != -1:
			self.ChatW("Verifique los caracteres de <Descripcion>")
			return

		questionDialog = uiCommon.QuestionDialog2()
		questionDialog.SetText1("Quiere enviar el mensaje a %s"%self.ReturnValues("nombre"))
		questionDialog.SetText2("Tasa de envio: 1000 yang.")
	...
	def __NameConfirmMail(self):
		if len(self.ReturnValues("nombre")) == 0:
			self.ChatW("<MailBox> Escriba un nombre.")
			return
